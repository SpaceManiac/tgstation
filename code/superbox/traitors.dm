...
/datum/antagonist/traitor/forge_single_generic_objective()
	...
	for (var/datum/objective/steal/S in objectives)
		if (S.steal_target == /obj/item/stock_parts/power_store/cell/syndicate)
...
/datum/objective/sabotage/proc/find_remaining()
	. = list()
	for (var/O in SSmachines.get_all_machines())
		...
		if (M.get_integrity() <= M.integrity_failure)
			continue
...
/obj/item/stock_parts/power_store/cell/syndicate
...
/obj/item/stock_parts/power_store/cell/syndicate/pre_attack(atom/A, mob/living/user, params)
	...
	var/obj/item/stock_parts/power_store/steal_cell
	if (istype(A, /obj/machinery/power/apc))
		var/obj/machinery/power/apc/apc = A
		if (apc.opened)
			return
		steal_cell = apc.cell
	else if (istype(A, /obj/item/stock_parts/power_store) && A != src)
		steal_cell = A
