...
/obj/machinery/geoscape/attackby(obj/item/W, mob/living/user, params)
	...
	if (!(ACCESS_COMMAND in W.GetAccess()))
...
/obj/machinery/geoscape/update_icon()
	...
	switch(state)
		...
		if (GEO_ON)
			...
			if(obj_flags & EMAGGED)
				display = mutable_appearance('icons/superbox/geoscape.dmi', "exalt")
			else
				display = mutable_appearance('icons/superbox/geoscape.dmi', "globe")
		if (GEO_TURNON)
